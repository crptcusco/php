...
require('../../../librerias.v2/utilidades.php');
require('model.php');
...
if (isset($resultado[0])) {
    ...
    $requerimientos = $cotizacion->mostrar_items_requerimientos($servicio_id);

    $requerimientos_str = '';
    if (is_array($requerimientos)) {
        $requerimientos_str .= '</w:t></w:r></w:p>';
        foreach($requerimientos as $row) {
            $requerimientos_str .= "<w:p><w:r><w:t>". $row['nombre'] . "</w:t></w:r></w:p>";
        }
...
//$section->addTextBreak(1,null,'My $ParagraphStyle');
    ...
    // print_r($items_cotizacion);
    // die();
    foreach ($items_cotizacion as $arreglo) {
        $arreglo['descr'] = utf8_decode($arreglo['descr']);
        $arreglo['descr'] = str_replace('&gt;', ' >> ', $arreglo['descr']);
        $arreglo['descr'] = Utilidades::sanear_complete_string($arreglo['descr']);
        $table->addRow(100);
        $table->addCell(4000, $styleCell)->addText($arreglo['descr'], null, $ParagraphCellStyle);
        $table->addCell(4000, $styleCell)->addText($arreglo['subtotal'], null, $ParagraphCellStyle);
    }
