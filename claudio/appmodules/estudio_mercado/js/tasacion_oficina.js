$(document).ready(function () {
    ...
    select_departamento_tipo_load();
    select_usuario_registra_load();

    $("#tipo_propiedad").change(function () {
        switch ($("#tipo_propiedad").val()) {
            case("EXCLUSIVA"):
            $('#row_estacionamiento').hide();

            $('#combo_ta_departamento_tipo_departamento').hide();

            $('#row_areas_complementarias').hide();

            $('#row_piso_cantidad').hide();
            $('#piso_cantidad').hide();
            //$('#vista_local_id').hide();
            break;  
            case("HORIZONTAL"):
            $('#row_estacionamiento').show();
            
            $('#combo_ta_departamento_tipo_departamento').show();
            $('#row_areas_complementarias').show();
            $('#row_piso_cantidad').show();
            $('#piso_cantidad').show();
            //$('#vista_local_id').show();
            break; 
            default:
    ...
    function select_departamento_tipo_load() {
        var enviar = {id: $('#tasacion_departamento_tipo_id').val()};
        // console.log(enviar);
        select_simple('../ajax/combos2/tasacion_departamento_tipo.php', '#tasacion_departamento_tipo_id', enviar);
    }
