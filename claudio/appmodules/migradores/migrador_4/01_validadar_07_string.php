...
echo "<h2 style='background-color: #64A4F1;'>-- STRING </h2>";

foreach ($tablas as $fuente => $destino) {
    string_process_table($fuente, $destino);
}
...
// ----------------------------------------------- function()
function string_data_field( $tabla, $campo, $value ) {
...
function string_data_tabla( $tabla, $campo ) {
  global $q;
  $q->fields = array(
    'nombre'=>''
  );
...
function string_process_field($tabla, $campo) {
    echo "<h4 style='background-color: #D2EDFE;'>-- $campo</h4>";
    /* $datos = string_data_tabla( $tabla, $campo ); */
    /* foreach ($datos as $row) { */
    /* 	$value = utf8_encode( $row['nombre'] ); */
    /* 	if( ! string_validate( $value ) ) { */
    /* 	    string_data_field( $tabla, $campo, $value ); */
    /* 	} */
    /* } */
}
function string_process_filter( $campos, $mapa) {
    $output= array();
    foreach ($campos as $campo) {
	if ( isset( $mapa[$campo] ) ) {
	    if ($mapa[$campo]['tipo']=='string' &&
		$mapa[$campo]['table']=='' &&
...
function string_process_table($fuente, $destino) {
    global $mapa;
    $campos = mapeador_info_fields_data( $fuente );
    $campos = string_process_filter( $campos, $mapa[$destino] );
    echo "<h3 style='background-color: #96D4FB;'>-- $fuente - $destino </h3>";
    foreach ($campos as $campo ) {
    	string_process_field( $fuente, $campo);
    }
}
